...
Settings are loaded from `.env` (local) or environment variables (Docker).
...
docker-compose exec backend uv run alembic -c alembic_trade_data/alembic_trade_data.ini revision --autogenerate -m "description"

# Apply migrations
docker-compose exec backend uv run alembic -c alembic_trade_data/alembic_trade_data.ini upgrade head

# Rollback last migration
docker-compose exec backend uv run alembic -c alembic_trade_data/alembic_trade_data.ini downgrade -1
```
...
docker-compose exec backend uv run pytest tests/unit/test_indicator_calculator.py -v

# Run locally
cd backend && pytest
```

Test fixtures provide synthetic OHLCV DataFrames (uptrend, downtrend, rising) to test indicator math and signal logic without any network or database dependencies.
